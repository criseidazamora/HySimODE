...
# ============================================================================
# Family 4 (Deterministic Oscillators) — Canonical Goodwin oscillator (literature)
# CORRECTED: tuned past Hopf -> sustained oscillations (limit cycle)
#   - high cooperativity (n=12)
#   - strong fold-change
#   - linear degradation only
#   - 5-step delay chain with slower conversion (distributed delay)
# ============================================================================
...
PARAMS = {
    # Transcription repression (increase nonlinearity + keep K in-range)
    "alpha_max": 180.0,   # molecules/min
    "alpha_leak": 0.25,   # molecules/min  (strong fold-change)
    "K_rep": 900.0,       # molecules
    "n_rep": 12.0,        # >= 10

    # Linear degradation (slow proteins -> more delay, helps oscillation sustain)
    "delta_m": 0.030,     # /min
    "delta_p": 0.0016,    # /min
    "delta_rep": 0.0016,  # /min

    # Translation (reduced to avoid huge initial spikes)
    "k_tl": 0.55,         # molecules/min per mRNA

    # 5-step delay chain (slower -> larger distributed delay)
    "k01": 0.0038,        # /min
    "k12": 0.0038,        # /min
    "k23": 0.0038,        # /min
    "k34": 0.0038,        # /min
...
# Softer ICs to avoid one-shot transients dominating the run
Y0 = [
    90.0,     # mRNA
    6000.0,   # P0
    3500.0,   # P1
...
if __name__ == "__main__":
    ...
    plt.title("osc03_goodwin_oscillator (tuned past Hopf: sustained oscillations; 5-step delay, n=12)")
    plt.legend()
    plt.tight_layout()
