...
# Model 39: stochosc06_activator_inhibitor_pulsed_input.py
# Activator–inhibitor oscillator driven by a time-dependent pulsed input.
#
# Deterministic ODE model (default): A is a fast activator with cooperative
# self-activation; I is a slower inhibitor induced by A. A is additionally
# suppressed by an I-dependent removal term (biochemically plausible:
# inhibitor-mediated inactivation / degradation).
#
# A periodic pulsed input u(t) transiently boosts activator synthesis,
# producing entrained oscillations (forced limit-cycle / relaxation-like),
# useful for RFC training because it generates sharp transitions and
# phase-resetting behavior without introducing nonphysical variables.
# ======================================================================
...
# Initial conditions (choose away from steady state to reveal dynamics quickly)
Y0 = [
    110.0,   # A_activator
    240.0,   # I_inhibitor
    780.0,   # N_pool
...
def pulsed_input(t: float, p: dict) -> float:
    ...
    # Choose a small window of pulses around t for efficiency
    # (Gaussian effectively zero beyond ~5 sigma)
    if sigma <= 0:
        return u0
...
if __name__ == "__main__":
    ...
    # ============================
    # Micro-audit (RFC labels)
    # label=1 if q80<200 and q99<200 else label=0
    # ============================
    labels = []
