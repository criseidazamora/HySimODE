...
# that drives CI cleavage/inactivation. Tuned so DEFAULT is lysogenic (CI-high).
# ============================================================================
...
PARAMS = {
    ...
    # CI autoregulation (activation by CI2) — moderate to avoid runaway CI2
    ...
    # Dimerization (mass-action) and undimerization — tuned to keep CI2 in a plausible range
    ...
    # RecA-driven CI cleavage (SOS induction) — DEFAULT near-off (lysogeny)
    # Key fix: reduce cleavage by ~10^3 so CIc does not accumulate in basal conditions.
    "k_cleave": 2.0e-7,   # 1/(molecule*min)
    "delta_CIc": 0.012,   # /min (clearance of cleaved fragments)

    # RecA dynamics: basal low (no DNA damage). Raising RecA0 externally can trigger switching.
    "RecA0": 120.0,
...
def dYdt(t, Y):
    ...
    # RecA-mediated cleavage of CI dimer (basal near-off)
...
if __name__ == "__main__":
    ...
    plt.title("grn07_lambda_switch (lysogenic default: CI-high, Cro-low; basal RecA ~ off)")
    plt.legend()
    plt.tight_layout()
