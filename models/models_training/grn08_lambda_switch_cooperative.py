...
# Model 28: Lambda switch with higher-order cooperativity + inducible (smooth) RecA pulse
#
# CI–Cro mutual repression with CI positive autoregulation.
# Explicit dimerization (CI2, Cro2).
# RecA drives CI2 cleavage -> CIc.
#
# Higher-order cooperativity via higher Hill exponents (n_rep, n_CI_act).
# Default lysogenic (CI-high, Cro-low) under basal RecA.
# Smooth RecA pulse can perturb / induce switching-like transient.
# ============================================================================
...
# ---------------------------
# What your RFC will use
# ---------------------------
var_names = SPECIES_NAMES

def model_odes(t, y):
    """
    IMPORTANT: If your pipeline imports model_odes, then it will see the PULSED system.
    """
    return dYdt_core(t, y, use_pulse=True)

# ---------------------------
# One-solver demonstration
# ---------------------------
def odes_stacked_one_solver(t, y16):
    """
    Integrate basal + pulsed in a SINGLE solve_ivp call by stacking states:
...
if __name__ == "__main__":
    ...
    # Micro-audit on BASAL (you can switch to pulsed if you prefer)
    labels = []
    ...
    plt.title("grn08_lambda_switch_cooperative (basal; lysogenic default expected)")
    plt.legend()
    plt.tight_layout()
    ...
    plt.xlabel("time [min]")
    plt.ylabel("molecules")
    plt.title("grn08_lambda_switch_cooperative (smooth RecA pulse; inducible perturbation)")
    plt.legend()
    plt.tight_layout()
    ...
    for i, name in enumerate(SPECIES_NAMES):
        ...
        auc_diff = np.trapezoid(diff, t)  # avoids np.trapz deprecation
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig("grn08_lambda_switch_cooperative_pulse_effect_delta.png", dpi=220)
    plt.close()
