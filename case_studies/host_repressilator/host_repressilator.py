...
def main():
    ...
    if PLOT:
        # Graficar TODAS las especies definidas en var_names (31)
        names = var_names  # usa tu lista completa
        index = {name: i for i, name in enumerate(var_names)}  # nombre -> índice

        n = len(names)
        ncols = 5                           # columnas del grid (ajústalo si quieres)
        ...
        # Ocultar ejes sobrantes si el grid es mayor que n
        for j in range(len(names), len(axes)):
            axes[j].axis('off')

        fig.suptitle('Host–Repressilator: todas las especies', y=0.995)
        fig.supxlabel('time')
