...
# -----------------------------------------------------------------------------
#  Description:
#      Trains and evaluates a Random Forest Classifier (RFC) to partition
#      biochemical model variables into stochastic (label=1) or deterministic
#      (label=0) regimes, using per-species features extracted from deterministic
#      simulations.
#
#  Key evaluation modes:
#      1) standard : stratified split by rows (species)  [fast, optimistic if
#                    species from the same model appear in both splits]
#      2) lomo     : Leave-One-Model-Out (LOMO) evaluation (grouped by model)
#                    [measures generalization to unseen ODE models]
#
#  Outputs:
#      - rfc_calibrated.joblib   : Calibrated RFC model (standard mode only,
#                                 or when --train_final is used)
#      - rfc_metadata.json       : Metadata for exported model
#      - lomo_metrics_by_model.csv / lomo_summary.json (LOMO mode)
#
...
def _eval_binary(y_true, proba, threshold=0.5):
    ...
    # Threshold-free metrics and MCC require both classes in y_true to be meaningful/defined
...
def run_lomo(args):
    ...
    # ---------------------------------------------------------------------
    # (C) Optional: Nested LOMO (outer = held-out model, inner = GroupKFold on TRAIN ONLY)
    # ---------------------------------------------------------------------
    nested_res = None
...
def main():
    ...
    # Nested LOMO (evaluation only; does not change final hyperparams unless you choose to)
