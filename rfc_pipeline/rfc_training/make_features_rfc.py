...
def compute_features(t: np.ndarray, Y: np.ndarray, obs_window: float):
    ...
    This function returns a dictionary per species containing:
      (i) robust windowed/normalized features used in the revised benchmark.
...
for fname in MODEL_FILES:
    ...
    mod = try_import_model(model_path)
    if mod is None:
        print(f"[SKIP] Model not found or failed to import: {model_path}")
        continue
    try:
    ...
    print(f"[OK] {model_path}: {nvars} species processed.")
