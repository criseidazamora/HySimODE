...
# Mutual repression between A and B with explicit dimerization (dA, dB).
# A also positively autoregulates its own expression.
# IndA and IndB are dynamic pulse-driven inducers that reduce effective
# DNA-binding activity and can trigger switching-like transients.
# ============================================================================
...
PARAMS = {
    ...
    # Repression thresholds for dimer-mediated mutual repression
    "K_Arep": 220.0,      # molecules (dB represses A)
    "K_Brep": 220.0,      # molecules (dA represses B)
    "n_rep": 3.0,         # cooperativity

    # Positive autoregulation of A by its own dimer
    ...
    # Dimerization, undimerization, and dimer turnover
    "k_dim": 1.1e-6,      # 1/(molecule*min)
    "k_undim": 0.050,     # /min
    "delta_d": 0.028,     # /min

    # Inducer potency scale for activity modulation
    "K_ind": 6000.0,      # molecules

    # ---- Inducers: dose-normalized pulses, decay, and weak consumption ----
    ...
    # Total delivered inducer doses (molecules)
    "doseA": 14000.0,
    "doseB": 9000.0,

    # Weak inducer consumption coupled to protein abundance
    "k_ind_consume": 1.5e-6,  # 1/(molecule*min)
}

# Initial condition biased toward the A-dominant state
Y0 = [
    60.0,   # mA
    55.0,   # mB
    420.0,  # pA
...
if __name__ == "__main__":
    ...
    plt.title("grn05_toggle_autoregulation (plausible ML-tuned pulses)")
    plt.legend()
    plt.tight_layout()
    plt.savefig("grn05_toggle_autoregulation_preview.png", dpi=220)
